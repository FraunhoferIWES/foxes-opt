...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-fig",
        help="Disable layout and flow plotting",
        action="store_true",
    )
    ...
    with engine:
        ...
        print()
        print(results)
        print(f"Population-based gradients: {not args.no_pop}")
        print(f"SLSQP iterations: {solver.scipy_results.nit}")
        print(f"SLSQP status: {solver.scipy_results.message}")
    ...
    if not args.no_fig:
        ...
        fig = output.get_mean_fig_xy(plot_data, fig=fig, ax=axs[1])
